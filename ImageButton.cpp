...
BOOL CImageButton::CreateBtmp(CWnd * pWnd,int nBitmapId,int nButtonId,const char * pszText,COLORREF crTextColor,eBitmapAlignment eAlignment)
{
	// Initialize
...
BOOL CImageButton::CreateIcon(CWnd * pWnd,int nIconId,int nButtonId,const char * pszText,COLORREF crTextColor,eBitmapAlignment eAlignment)
{
	// Initialize
...
BOOL CImageButton::CreateRect(CWnd * pWnd,int nButtonId,COLORREF crFillColor,const char * pszText,eBitmapAlignment eAlignment,int iWidth,int iHeight)
{
	// Initialize
...
BOOL CImageButton::CreateText(CWnd * pWnd,int nButtonId,const char * pszText,COLORREF crTextColor)
{
	m_crTextColor = crTextColor;
	m_eAlignment = LEFT;
	m_iWidth = 0;
...
void CImageButton::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
	...
	if (csButtonText.GetLength())
	{
		...
		csTextExtent.Remove('&');
//		Size = dc.GetTextExtent(csTextExtent);
		...
		if (m_bImage)
		{
			...
			if (bDisabled)
				nFlags |= DSS_DISABLED;
			dc.DrawState(CPoint(nBmpStart,nYBmp + nOff),CSize(m_iWidth,m_iHeight),m_BitmapIcon,nFlags,(HBRUSH)0);
		}
		else if (m_bRect)
		{
